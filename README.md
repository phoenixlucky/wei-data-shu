...
| 工具 | `wei_data_shu.utils` | `fn_timer`, `generate_password`, `search_colors`, `color_records`, `mav_colors`, `in_notebook` | 函数计时器、安全密码生成、颜色检索、Jupyter 环境探测 |
...
打 `wei-data-shu-<版本>` 格式的 tag（例如 `wei-data-shu-0.9.0`）并推送，CI 会自动构建并发布到 PyPI：

```bash
git tag wei-data-shu-0.9.0
git push origin wei-data-shu-0.9.0
```
